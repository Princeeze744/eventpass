...
type Data = {
  ...
  events: Ev[];
  users: Usr[];
  byType?: [string, { count: number; guests: number }][];
  byState?: [string, { count: number; guests: number }][];
};
        ...
        {((data?.byType?.length ?? 0) > 0) && (
          <div className="mt-3 grid gap-3 lg:grid-cols-2">
            <div className={`${card} p-5`}>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[#c9a227] font-[family-name:var(--font-sans)]">Events by type</p>
              <div className="mt-4 space-y-2.5">
                {(data?.byType || []).map(([k, v]) => (
                  <div key={k} className="flex items-center justify-between border-b border-white/[0.04] pb-2 text-[12px] font-[family-name:var(--font-sans)]">
                    <span className="text-white/65">{k}</span>
                    <span className="text-white/40">{v.count} {v.count === 1 ? "event" : "events"} &middot; {v.guests.toLocaleString()} guests</span>
                  </div>
                ))}
              </div>
            </div>
            <div className={`${card} p-5`}>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[#c9a227] font-[family-name:var(--font-sans)]">Events by state</p>
              <div className="mt-4 space-y-2.5">
                {(data?.byState || []).map(([k, v]) => (
                  <div key={k} className="flex items-center justify-between border-b border-white/[0.04] pb-2 text-[12px] font-[family-name:var(--font-sans)]">
                    <span className="text-white/65">{k}</span>
                    <span className="text-white/40">{v.count} {v.count === 1 ? "event" : "events"} &middot; {v.guests.toLocaleString()} guests</span>
                  </div>
